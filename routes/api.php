...
$api->version('v1', function($api){
    ...
    $api->group(['middleware' => ['throttle:60,1', 'bindings'], 'namespace' => 'App\Http\Controllers'], function($api) {
        ...
        $api->group(['prefix' => 'constituencies'], function ($api) {
            $api->get('/', 'Api\Constituencies\ApiConstituenciesController@index');
            $api->get('/{id}', 'Api\Constituencies\ApiConstituenciesController@show');
        });
        ...
        $api->group(['prefix' => 'companies'],function ($api) {
            $api->get('/', 'Api\Company\ApiCompanyController@index');
            $api->get('/{id}', 'Api\Company\ApiCompanyController@show');
        ...
        $api->group(['middleware' => ['auth:api'], ], function ($api) {
            ...
            //companies routes
            $api->group(['prefix' => 'companies'],function ($api) {
                $api->post('/', 'Api\Company\ApiCompanyController@store');
                $api->put('/{id}', 'Api\Company\ApiCompanyController@update');
                $api->patch('/{id}', 'Api\Company\ApiCompanyController@update');
                $api->delete('/{id}', 'Api\Company\ApiCompanyController@destroy');
            });

            //auth companies routes
            $api->group(['prefix' => 'authcompanies'],function ($api) {
                $api->get('/', 'Api\Company\ApiCompanyController@authUserCompanies');
                $api->get('/userCompanyStatus/{company_id}', 'Api\Company\ApiCompanyController@userCompanyStatus');
                $api->get('/{id}', 'Api\Company\ApiCompanyController@show');
            });

            //company join request routes
            $api->group(['prefix' => 'companyjoinrequests'],function ($api) {
                $api->post('/', 'Api\Company\ApiCompanyJoinRequestController@store');
                $api->post('/sendjoinrequest/{company_id}', 'Api\Company\ApiCompanyJoinRequestController@sendJoinRequest');
                $api->put('/{id}', 'Api\Company\ApiCompanyJoinRequestController@update');
                $api->patch('/{id}', 'Api\Company\ApiCompanyJoinRequestController@update');
                $api->delete('/{id}', 'Api\Company\ApiCompanyJoinRequestController@destroy');
            });
